...
Import-CSV ".\computers.csv" | ForEach-Object {
	$computer = $_.computer
	if((Test-OnlineFast -ComputerName $computer).Online -eq "True") {
		write-host $computer " is pinging"
		add-content ".\success.txt" $computer
	} else {
		write-host $computer " is not pinging"
		add-content ".\failures.txt" $computer
	}
}
